...
public class OnlineUsersFlatfile extends OnlineUsersDataSource {
	...
	public synchronized boolean init() {
		return (initFile(OnlineUsers.flatfile) && initFile(OnlineUsers.flatfileData) && initTemplateFile());
	}
	...
	public synchronized boolean addUser(String username) {
		...
		try {
			bw = new BufferedWriter(new FileWriter(OnlineUsers.flatfileData, true));
			bw.newLine();
			bw.append(username+":"+System.currentTimeMillis()+":"+String.valueOf(ONLINE));
		} catch (Exception e2) {
			log.log(Level.SEVERE, "Exception while writing new user to " + OnlineUsers.flatfileData, e2);
		} finally {
			try {
				if (bw != null) {
					bw.close();
	...
	public synchronized boolean removeUser(String username) {
		FileWriter writer = null;
		try {
			BufferedReader reader = new BufferedReader(new FileReader(new File(OnlineUsers.flatfileData)));
			String line = "";
			...
			writer = new FileWriter(OnlineUsers.flatfileData);
			writer.write(toSave.toString());
		} catch (Exception e1) {
			log.log(Level.SEVERE, "Exception while removing player '" + username + "' from " + OnlineUsers.flatfileData, e1);
		} finally {
			try {
				if (writer != null) {
					writer.close();
	...
	public boolean setAllOffline() {
		...
		try {
			...
			log.log(Level.SEVERE, "Exception setting all offline from " + OnlineUsers.flatfileData, e1);
		} finally {
			try {
				if (writer != null) {
					writer.close();
	...
	private synchronized boolean regenFlatFile() {
		FileWriter writer = null;
		try {
			BufferedReader templatereader = new BufferedReader(new FileReader(new File(OnlineUsers.flatfileTemplate)));
			BufferedReader datareader = new BufferedReader(new FileReader(new File(OnlineUsers.flatfileData)));
			String line = "";
			...
			writer = new FileWriter(OnlineUsers.flatfile);
